...
fit.mean<-function(num.knots,bdeg,pord,y,ar.ord){
    ...
    #AIC
    AIC=n*log(sigma2.hat)+2*(ar.ord+edf[count])
    
    list(yhat=yhat,edf=edf,gcv=gcv,index=index,error=error,L=L,ytilde=ytilde,Btilde=Btilde,AIC=AIC)
}
...
CochranOrcutt<-function(num.knots,bdeg,pord,y){
    ...
    error<-fit$error
    covnoise=mycorMatrix(error,ar.ord)
    R.hat=covnoise$R
