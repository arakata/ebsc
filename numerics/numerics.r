...
WINDOWS = T # set to T if code is run on Windows machine
...
SAVE_PLOT = F # should plots be saved to disk? (if F then they are displayed)
...
functions[[3]] = function(x) (6*dbeta(x,30,17)+4*dbeta(x,3,11))/10/3.5
...
if(RECOMPUTE)
  ...
  save(extracted, file = "./results/results_MSEs_all_stacked.csv")
}else
